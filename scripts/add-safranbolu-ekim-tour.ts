/**
 * Safranbolu Amasra Turu — 10-11 Ekim 2026 Seed Script
 * Kullanım: npx tsx scripts/add-safranbolu-ekim-tour.ts
 */
...
const safranboluEkimTour = {
  name: 'Safranbolu Amasra Turu - 10-11 Ekim 2026',
...
Tur programında bölge yoğunluğu, trafik, hava koşulları sebebiyle sıralama değişikliği olabilir.`,
  ...
  slug: 'safranbolu-amasra-turu-10-11-ekim-2026',
  duration: '2 Gün 1 Gece',
  price: 5750,
  destination: 'Safranbolu, Amasra, Yörük Köyü, Cam Teras, Bartın',
...
async function addSafranboluEkimTour() {
  try {
    console.log("MongoDB'ye bağlanılıyor...");
    await mongoose.connect(MONGODB_URI as string);
    console.log('✅ MongoDB bağlantısı başarılı');

    const existingTour = await Tour.findOne({ slug: safranboluEkimTour.slug });

    if (existingTour) {
      console.log('⚠️ Bu tur zaten mevcut, güncelleniyor:', existingTour.name);
      Object.assign(existingTour, safranboluEkimTour);
      await existingTour.save();
      console.log('✅ Tur başarıyla güncellendi');
    } else {
      const newTour = new Tour(safranboluEkimTour);
      await newTour.save();
      console.log('✅ Tur başarıyla eklendi:', newTour.name);
    }

    console.log('\n📊 Tur Detayları:');
    console.log('- Tarih: 10-11 Ekim 2026');
    console.log('- Fiyat:', safranboluEkimTour.price, 'TL');
    console.log('- Tür: Konaklamalı / Yurtiçi / Öne çıkan');
    console.log('- URL: /tours/' + safranboluEkimTour.slug);
  } catch (error) {
