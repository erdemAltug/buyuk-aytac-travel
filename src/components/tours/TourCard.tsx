...
export default function TourCard({
  ...
  const formattedDate = tour.startDate ? formatDateShort(tour.startDate) : null;
      ...
      <article className="flex h-full min-h-0 flex-col overflow-hidden rounded-2xl border border-slate-100 bg-white shadow-sm transition-shadow duration-300 hover:shadow-xl">
        ...
        >
          <div className="relative aspect-square w-full shrink-0 overflow-hidden bg-slate-100">
            ...
            {tour.discountRate && tour.discountRate > 0 && (
              <div className="absolute bottom-3 right-3 z-10 rounded-full bg-red-500 px-3 py-1 text-xs font-bold text-white">
        ...
        <div className="mt-auto shrink-0 border-t border-slate-100 px-4 pb-4 pt-3">
          <div className="mb-3 flex min-h-[2.75rem] items-start justify-between gap-3">
            <div>
              {tour.discountRate && tour.discountRate > 0 ? (
